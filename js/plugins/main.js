...
function CanvasState(canvas) {
			...
			for (var i = l-1; i >= 0; i--) {
				if (shapes[i].contains(mx, my)) {
					if (myState.lineCreation) {
						...
						var newEdge = new CanvasEdge(startShape, shapes[i], myState);
	...
	canvas.addEventListener('drop', function(e) {
			...
			myState.addShape(new CanvasNode(Math.round((mouse.x - 10)/SNAPPING)*SNAPPING, Math.round((mouse.y - 10)/SNAPPING)*SNAPPING, undefined, undefined, stencilType, data, myState));
...
CanvasState.prototype.remove = function(element) {
	...
	} else if (element instanceof CanvasEdge) {
...
CanvasState.prototype.serialize = function() {
	...
	nodes.each(function(node, i) {
		var o = {};
		$H(node.data).each(function(p) {
			if ((p.value||{}).getValue instanceof Function) {
				o[p.key] = p.value.getValue();
			} else {
				o[p.key] = p.value.value || p.value;
			}
		})
		operators[node.id] = o;
	});
...
CanvasState.prototype.deserialize = function(json, myState) {
	...
	$H(obj.operators).each(function(p) {
		var stencil = jQuery.extend(true, {}, jsons[p.value.type]);
		...
		myState.addShape(new CanvasNode(
		// console.log(
			getX(p),
			getY(p),
			undefined, undefined,
	...
	obj.edges.each(function(edge) {
		myState.addEdge(new CanvasEdge(
		// console.log(
...
function init() {
	...
	// XXX Initial load of a model
	// s.deserialize(defaultJSON, s);
